...
/**
 * A safe status report for debugging a deployment: is the config present, what
 * KIND of key is it, which project host, and what does Supabase actually say.
 * Never returns key material -- only its shape and what the server can reach.
 */
async function diagnose(cfg) {
  ...
  const out = {
    ...
    missing,
    keyKind: keyKind(cfg.key),
    keyLength: cfg.key.length,
    projectHost: cfg.url.replace(/^https?:\/\//, '') || null,
    supabase: null
  };
  if (!out.configured) return out;
  try {
    const r = await supabaseFetch(cfg, '/rest/v1/leaderboard?select=name&limit=1');
    out.supabase = { status: r.status, ok: r.ok };
    if (!r.ok) out.supabase.body = (await r.text().catch(() => '')).slice(0, 160);
  } catch (err) {
    out.supabase = { status: null, ok: false, threw: err.name + ': ' + err.message.slice(0, 120) };
  }
  return out;
}
